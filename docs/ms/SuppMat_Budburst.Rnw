...
print(xtable(meanzb.table, caption = "Summary of mixed effect model of budburst day by species. See also Figure S2."), 
...
print(xtable(meanzl.table, caption = "Summary of mixed effect model of leafout day by species. See also Figure S3."), 
      include.rownames=TRUE, 
      caption.placement = "top",
      sanitize.rownames.function = function(x) {x})
@
...
\label{figS2}
\includegraphics[width=1\textwidth]{Fig1_bb_lo+sp} % built in Additional Plots and Processing.R
\end{figure}
...
\label{figS3}
\includegraphics[width=1\textwidth]{Fig1_bb_lo+sp}
\end{figure}
...
\label{figS5}
\includegraphics[width=1\textwidth]{Fig2_4panel_ZoomSupp}
\end{figure}
