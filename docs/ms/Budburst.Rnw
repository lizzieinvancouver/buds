...
Researchers have long recognized three major environmental cues that drive spring phenology: forcing (warm temperatures, occurring predominantly in the spring), chilling (cooler temperatures, occurring predominantly in the fall-winter and possibly spring) and photoperiod (CITE). These cues are known to interact and possibly modify one another in some species. For example, insufficient amount of chilling may be offset by additional forcing (CITE); photoperiod and forcing are known to interact in driving spring phenology in woody species---especially through their complex impacts on dormancy release (CITE CHUINE). Yet, while these complexities have been identified in some species, a growing body of hypotheses and experimental studies, focused on woody species, has suggested many species are dominated by one cue and may lack any response to other cues (CITES). If this is true it has critical implications for predicting responses to climate change, as a species dominated by a forcing cue could march forever earlier in their leafout with continued warming, while species with strong photoperiod cues could stop advancing at some threshold point (CITES). \\

% START HERE! Read all of this, then try to decide how to tackle this paragraph
Alongside this growing interest in how cues co-vary within species is an aim to understand and predict variation in cues across species. Research here has focused on attributes of species: origin of species on shorter and longer timescales (CITEZOHNER?), (CITES) the successional stage (pioneer or climax communities) to which species traditionally belong (CITES) and a variety of possibly related traits (CITES). Most of these studies hinge on an assumption that phenology, by helping define the temporal niche of a species, is a critical axis of plant species assemble within one community. LINK NOW TO INVASIONS.... 
...
% Maybe just say: People have focused on invasions and succession, but this all boils down to assuming phenology is critical to plant coexistence? Or flip it around like this outline: 
(1) Temporal niche baby
(2) invasions
(3) succession? (Do we even want to mention this?! Or save for conclusions)
(4) Crazy shit about STV or such .... 
...
(1) Species would show staggered cues (driving staggered leafout across the season).
(2) Within species cues would trade-off, such that some species would be dominated by one or another cue, while other would show a mix of cues. 
\end{itemize}


%% Below is original from Dan: 
Woody plant  The crucial role that phenology plays in ecosystem processes, and the wealth of observational data highlighting how rapidly plant and animal phenology are advancing \cite{Menzel:2006} has led to increased interest in better understanding and prediction of how plant phenology will shift with continued climate change.
