...
Timing of spring phenology is critical for understanding net ecosystem assimilation at the forest scale, as maximizing the length of the growing season and minimizing risk of tissue loss due to early spring freezing depends on accurate timing of budburst and leafout (Basler \& Korner 2014).  
...
hftable <- xtable(hfallcalc, caption = "Chill unit calculations")
...
print(xtable(traitlm.t, caption = "Trees, leafout"),
      caption.placement = "top",
      sanitize.text.function = function(x) {x})
...
print(xtable(traitlm.s, caption = "Shrubs, leafout"),
      caption.placement = "top",
      sanitize.text.function = function(x) {x})
