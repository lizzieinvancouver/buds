...
Phenology is also one of the major biological indicators of climate change, with plant phenology shifting across the globe 4-6 days/C with warming (CITE). While this average response is strikingly consistent across species, space and time (CITE), it masks tremendous variation. Variation is extreme when examined across species (CITE), but additional variation can be seen within species across time (CITEFU). Understanding this variation has been the goal of much recent work (CITES), with researchers focuses on two major linked areas: (1) identifying and quantifying the environmental cues that drive spring phenology (i.e., leafout and budburst), and (2) identifying what drives variation in cues between different species. \\

Para here on cues. \\

Para here on species variation. \\
