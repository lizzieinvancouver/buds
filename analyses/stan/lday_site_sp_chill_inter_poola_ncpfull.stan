...
// Adding non-centered parameterization (NCP or ncp) on all things with a ~norma(mu, sigma)
data {
  int<lower=0> N;
  int<lower=0> n_sp;
  int<lower=0> n_site;
...
/*	mu_b_warm ~ normal(0, 35); // 100 = 3 months on either side. Narrow down to 35
	...
	mu_b_site ~ normal(0, 35); */
        ...
        mu_b_warm_ncp ~ normal(0, 35); 
	mu_b_photo_ncp ~ normal(0, 35);
	mu_b_chill1_ncp ~ normal(0, 35);
	mu_b_chill2_ncp ~ normal(0, 35);
	mu_b_site_ncp ~ normal(0, 35);
