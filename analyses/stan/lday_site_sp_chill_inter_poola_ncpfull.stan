...
// Stan model by Dan Flynn following Meetup 2016-03-28
...
// And it includes non-centered parameterization (NCP or ncp) on all things with a ~norma(mu, sigma)
...
parameters {
  ...
  vector[n_sp] b_chill2_ncp;
  vector[n_sp] b_site_ncp;
...
transformed parameters { // Vectorize: Won't save time probably here (no scalar x vector)
  ...
  b_chill2 = mu_b_chill2 + sigma_b_chill2*b_chill2_ncp;
  b_site = mu_b_site + sigma_b_site*b_site_ncp;
...
/*	a_sp ~ normal(mu_a, sigma_a);  */  // Delete because all in NCP now

        a_sp_ncp ~ normal(0, 35);
	
/*	b_warm ~ normal(mu_b_warm, sigma_b_warm);  // Delete because all in NCP now
	...
	b_chill2_ncp ~ normal(0, 35);
	b_site_ncp ~ normal(0, 35);
