...
// Now with chilling levels as dummy variables for each level
...
transformed parameters {
	...
	for(i in 1:N){
		...
		y_hat[i] <- a[sp[i]] + b_warm[sp[i]] * warm[i] + b_photo[sp[i]] * photo[i] + b_chill1[sp[i]] * chill1[i] + b_chill2[sp[i]] * chill2[i] + b_chill3[sp[i]] * chill3[i];
